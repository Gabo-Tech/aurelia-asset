    ...
    <Card className="border-border/60 mt-5">
      ...
      <CardContent>
        ...
        <div className="flex flex-wrap items-center gap-2 text-xs text-muted-foreground mb-3">
          ...
          >
            {t("more.tpNetInvested")}:{" "}
            {privacy
              ? MASK
              : `${totals.net >= 0 ? "+" : "−"}${formatMoney(Math.abs(totals.net), currency)}`}
          </span>
          <span className="ml-auto">
            {filtered.length} {t("more.tpCountSuffix")}
          </span>
        </div>

        {filtered.length === 0 ? (
          <div className="py-10 text-center text-sm text-muted-foreground">
            {state.transactions.length === 0 ? t("more.tpEmpty") : t("more.tpEmptyFilters")}
          </div>
        ) : (
              ...
              <tbody className="divide-y divide-border/40 [&>tr>td]:px-3">
                {[...filtered]
                  ...
                  .map((tx) => {
                    ...
                    const total =
                      tx.quantity * tx.pricePerUnit + (tx.fees ?? 0) * (tx.kind === "buy" ? 1 : -1);
                    return (
                      <tr key={tx.id}>
                        ...
                        <td className="py-2.5">
                          ...
                          {tx.notes && (
                            <div
                              className="text-[11px] text-muted-foreground mt-0.5 truncate max-w-[260px]"
                              title={tx.notes}
                            >
                              {tx.notes}
                            </div>
                        ...
                        <td className="py-2.5 text-right tabular-nums">
                          {formatMoney(tx.pricePerUnit, cur)}
                        </td>
                        ...
                        >
                          {privacy
                            ? MASK
                            : (tx.kind === "buy" ? "+" : "−") + mask(total, tx.currency)}
                        </td>
                        <td>
                          <DropdownMenu>
                            ...
                            <DropdownMenuContent align="end">
                              ...
                              <DropdownMenuItem
                                ...
                                onClick={() => {
                                  ...
                                  toast.success(t("more.tpRemoved"));
                                }}
                              >
