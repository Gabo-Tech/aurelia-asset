...
export async function isNativeDesktop(): Promise<boolean> {
  ...
  // React Native WebView bridge: local model download is available even on mobile.
  if (typeof window !== "undefined" && (window as Window & { __RN_BRIDGE__?: boolean }).__RN_BRIDGE__) {
    return true;
  }
  try {
