const en = {
  common: {
    save: "Save",
    cancel: "Cancel",
    delete: "Delete",
    edit: "Edit",
    add: "Add",
    close: "Close",
    confirm: "Confirm",
    loading: "Loading…",
    search: "Search",
    name: "Name",
    color: "Color",
    type: "Type",
    amount: "Amount",
    date: "Date",
    category: "Category",
    description: "Description",
    currency: "Currency",
    actions: "Actions",
    total: "Total",
    none: "None",
    all: "All",
    yes: "Yes",
    no: "No",
    optional: "(optional)",
    clear: "Clear",
    create: "Create",
    refresh: "Refresh",
  },
  nav: {
    dashboard: "Dashboard",
    holdings: "Holdings",
    performance: "Performance",
    cashflow: "Cashflow",
    planning: "Planning",
    assistant: "Assistant",
    settings: "Settings",
    more: "More",
    short: {
      dashboard: "Home",
      holdings: "Assets",
      performance: "Perf",
      cashflow: "Flow",
      planning: "Plan",
      assistant: "AI",
      settings: "Setup",
      more: "More",
    },
  },
  assistant: {
    metaTitle: "AI Assistant · Aurelia",
    metaDesc:
      "Talk to a fully offline finance assistant. Log expenses by voice or text and get saving tips from your own data.",
    title: "AI Assistant",
    subtitle: "Fully offline · your data never leaves this device",
    onDeviceEngine: "On-device assistant",
    localLlm: "Local LLM",
    localLlmQwen: "Local LLM (Qwen)",
    genericError: "Sorry, something went wrong. Please try again.",
    voiceUnavailable: "Voice input isn't available on this device.",
    noSpeech: "I didn't hear anything. Tap the mic and try again.",
    sttError: "Couldn't capture audio. Check mic permissions.",
    expenseAdded: "Expense added",
    expenseAddedMsg: "Done — I added {{amount}} to {{category}}.",
    chipAddExpense: "I spent 12 on lunch",
    chipFoodMonth: "How much did I spend on food this month?",
    chipRecent: "Show my recent transactions",
    chipBudget: "How's my budget?",
    chipTips: "Give me saving tips",
    muteVoice: "Mute voice",
    unmuteVoice: "Enable voice",
    clear: "Clear chat",
    confirmAdd: "Add it",
    cancel: "Cancel",
    retry: "Retry",
    thinking: "Thinking…",
    transcribing: "Transcribing…",
    speaking: "Speaking…",
    listeningPlaceholder: "Listening…",
    inputPlaceholder: "Describe an expense or ask a question…",
    stopRecording: "Stop",
    startRecording: "Speak",
    sendBtn: "Send",
    voiceHint: "Voice input requires the native app or a supported browser. Text works everywhere.",
    emptyTitle: "Your private money assistant",
    emptyBody:
      'Ask about your net worth, spending, or say "give me financial advice". You can also log expenses like "I spent 45 on groceries" — I\'ll confirm before saving.',
    backend: {
      greeting:
        "Hi! I'm your personal financial advisor. Ask about net worth, spending, budgets, goals, or say \"give me advice\" — and I can log expenses when you describe them.",
      help: "I can:\n• Analyse your finances and coach you (cut costs, grow income, invest surplus)\n• Report net worth, portfolio, liquidity, and debt\n• Summarise spending, budgets, goals, and loans\n• Log expenses you describe (I'll confirm first)\nEverything runs on your device using your real data.",
      thanks: "You're welcome! Ask anytime for a check-in on your finances.",
      positive: "Glad that helped! Want tips on spending, investing, or growing income next?",
      goodbye: "See you! I'll be here whenever you want a money check-in.",
      ack: "Got it. What else can I help you with?",
      howAreYou:
        "Ready to dig into your numbers — spending, net worth, goals, the works. What should we look at?",
      unknown:
        'I didn\'t quite catch that. Try "What\'s my net worth?", "how much did I spend on food this month?", "give me financial advice", or "spent 8.50 on coffee".',
      done: "Done.",
      expenseConfirm: "Add a {{amount}} {{category}} expense for {{when}}? Confirm below.",
      expenseConfirmToday: "today",
      expenseParseError:
        'Sorry, I couldn\'t understand that expense. Try e.g. "spent 12 on lunch yesterday".',
      updateHint:
        "To correct an entry, tell me the new amount or category and I'll add the fix, or edit it directly in Cashflow.",
      processError:
        "Sorry, I couldn't process that right now. Please try again, or rephrase your request.",
      maxStepsReply:
        "Here's what I found based on your data. Ask me anything else about your finances.",
      advice: {
        disclaimer:
          "Educational guidance only — not professional financial advice. I analyse your local data to suggest ideas; consider your own situation and consult a qualified advisor for major decisions.",
        intro: "Here's personalised guidance based on your data:",
        overspending:
          "You're spending {{amount}} more than you earned this month — the top driver is {{category}}.",
        netPositive:
          "You're net positive by {{amount}} this month — consider moving part of it into savings or goals.",
        lowSavings:
          "You're saving about {{rate}}% of income this month. If you can, aim for 15–20% — even {{amount}}/month would help.",
        strongSavings:
          "Strong savings rate at {{rate}}% this month — consider directing some of that surplus toward goals or long-term investments.",
        lowIncome:
          "Expenses are using {{pct}}% of your income this month. Trimming fixed costs or growing income (raise, better role, side income, skills) could ease the pressure.",
        tightCashflow:
          "Expenses are {{pct}}% of income — there's little buffer. Cap discretionary spending and look for one income boost this quarter.",
        growIncome:
          "Your surplus is thin relative to income. Alongside cutting costs, consider raising earning power (negotiation, promotion path, freelance, or a higher-paying role).",
        topCategory:
          "{{name}} is {{pct}}% of your spending ({{amount}}). Trimming it 10% would free {{savings}}/month.",
        secondCategory:
          "{{name}} is another big slice at {{pct}}% ({{amount}}) — a good second place to trim.",
        highDebt:
          "You owe {{debt}} on credit cards. Paying that down first could free cash for saving and investing.",
        highAprLoan:
          'Loan "{{name}}" costs {{apr}}% APR on {{principal}}. Extra payments here usually beat low-yield cash sitting idle.',
        excessCash:
          "You have {{liquidity}} in liquidity (~{{months}} months of expenses). Keeping ~{{reserve}} as an emergency buffer is sensible; {{investable}} could gradually move into investments.",
        lowInvestment:
          "About {{pct}}% of your wealth is in cash/liquidity ({{liquidity}} liquid vs {{invested}} invested). If your emergency fund is covered, consider investing a portion for long-term growth.",
        startInvesting:
          "You have solid liquidity but no holdings yet. After an emergency buffer (3–6 months of expenses), start a simple long-term investment plan — diversified funds, not stock-picking.",
        balancedAllocation:
          "Your allocation looks balanced — {{invested}} invested and {{liquid}} in cash/liquid holdings. Keep contributing steadily.",
        overBudget:
          "You're over budget on {{categories}}. Ease off there for the rest of the month.",
        onTrack:
          "You're within your \"{{plan}}\" budget so far — nice. Keep the pace and you'll land under {{limit}}.",
        goalOnTrack:
          'Goal "{{name}}" is {{pct}}% complete ({{current}} of {{target}}) — solid progress.',
        goalBehind:
          'Goal "{{name}}" is only {{pct}}% done ({{current}} of {{target}}). About {{monthly}}/month would close the gap in a year.',
        keepTracking:
          "Keep logging income and expenses — I'll refine these suggestions as your picture gets clearer.",
        empty:
          "Add a few expenses, holdings, and a budget in Planning, and I'll give you tailored guidance based on your real finances.",
        unknownCategory: "your largest category",
      },
      periods: {
        this_month: "this month",
        last_month: "last month",
        this_week: "this week",
        this_year: "this year",
        ytd: "year to date",
        all: "all time",
      },
      tools: {
        spentCategory: "Spent {{total}} on {{category}} ({{period}}).",
        spentTotal: "Total spent {{total}} ({{period}}). Top: {{top}}.",
        noTransactions: "No transactions yet.",
        noBudget: "No budget set up yet.",
        noHoldings: "No holdings yet.",
        noGoals: "No savings goals set up yet.",
        noLoans: "No loans tracked yet.",
        budgetStatus: 'Budget "{{name}}": {{spent}} of {{limit}} used.',
        overOn: " Over on: {{list}}.",
        onTrack: " On track.",
        unknownTool: "Unknown tool.",
        txLine: "{{date}} {{kind}} {{category}} {{amount}}",
        netWorth:
          "Net worth {{netWorth}} = portfolio {{portfolio}} (invested {{invested}}, cash-like {{cashLike}}) + liquidity {{liquidity}} − card debt {{debt}}.{{savingsRate}}",
        savingsRate: " Savings rate this month: {{rate}}%.",
        portfolio: "Portfolio {{total}}. Holdings: {{lines}}.",
        holdingLine: "{{symbol}} {{name}} ({{type}}, {{horizon}}) {{value}}",
        goalLine: "{{name}}: {{current}} / {{target}} ({{pct}}%)",
        loanLine: "{{name}}: {{principal}} at {{apr}}% APR over {{months}} months",
      },
      voice: {
        recognitionUnavailable: "Speech recognition unavailable.",
        recognitionError: "Recognition error.",
        transcriptionFailed: "Transcription failed.",
        micDenied: "Microphone permission denied or unavailable.",
      },
    },
  },
  errors: {
    notFoundTitle: "Page not found",
    notFoundBody: "The page you're looking for doesn't exist.",
    loadFailedTitle: "This page didn't load",
    loadFailedBody: "Something went wrong.",
    tryAgain: "Try again",
    goHome: "Go home",
  },
  shell: {
    brand: "Aurelia",
    brandTagline: "Your money, on your device",
    showValues: "Show values",
    hideValues: "Hide values",
    footerNote: "Local-only · your data stays on this device",
    sourceCode: "Source code",
  },
  landing: {
    headerTagline: "Private · On your device · Free",
    openApp: "Open Aurelia",
    logoAlt: "Aurelia logo",
    nav: {
      features: "Features",
      how: "How it works",
      downloads: "Download",
      faq: "FAQ",
    },
    hero: {
      badge: "No account. No cloud sync. No telemetry.",
      titleStart: "Private portfolio tracking.",
      titleHighlight: "On your device",
      subtitle:
        "Track stocks, ETFs, crypto, cash and net worth in one place. Aurelia encrypts your data on this device, never asks who you are, and never syncs to the cloud. Free and open source.",
      ctaPrimary: "Open Aurelia",
      ctaSecondary: "See what you get",
      ctaDownload: "Download the app",
      screenshotAlt: "Aurelia dashboard with allocation chart and cashflow summary",
    },
    proof: {
      zeroAccounts: "Data stays on your device",
      offline: "Web, Linux and Android today",
      multiCurrency: "Every currency you use",
      openSource: "AGPL — audit the source",
    },
    features: {
      heading: "Everything you need to feel in control",
      subheading:
        "Portfolio, cashflow and planning in one private app. No spreadsheet required.",
      items: {
        assets: {
          title: "All your wealth, one view",
          body: "Stocks, ETFs, crypto, metals, cash and custom holdings in one place, converted into the currency you actually think in.",
        },
        performance: {
          title: "Know how you're really doing",
          body: "Live prices, history charts and time-weighted returns show whether your portfolio is growing, not just moving.",
        },
        sankey: {
          title: "See where your money flows",
          body: "Income, expenses, savings and investments in a Sankey diagram you can reorder and reshape.",
        },
        categories: {
          title: "Plan the months ahead",
          body: "Budgets, savings goals, loan schedules and a 24-month forecast turn today's numbers into tomorrow's plan.",
        },
        private: {
          title: "Private by default",
          body: "AES-GCM encryption on your device. No account, no cloud sync, no analytics on what you own. Quotes and FX may use the network — your portfolio never leaves.",
        },
        elegant: {
          title: "Made for your phone first",
          body: "Thumb-friendly screens on mobile, a full desktop layout on larger screens, and six languages including Valencià.",
        },
      },
    },
    how: {
      heading: "From zero to clarity in 60 seconds",
      cta: "Try it free",
      steps: {
        one: {
          title: "Open it",
          body: "No signup. Use the web app in your browser, or install the Linux or Android build. Everything runs on your device.",
        },
        two: {
          title: "Add what you own and what you spend",
          body: "Search a ticker, log a paycheck, add a card or an installment plan in your own categories and currencies.",
        },
        three: {
          title: "Watch the full picture come alive",
          body: "Allocation, net worth, cashflow and forecast update as prices move and life happens.",
        },
      },
    },
    comparison: {
      heading: "Why people leave cloud trackers behind",
      subheading: "What you get here that big-name apps quietly take away.",
      feature: "What you get",
      us: "Aurelia",
      them: "Typical cloud app",
      rows: {
        signup: "Skip the signup",
        servers: "Data stays on your device",
        free: "Free forever",
        assets: "Stocks, ETFs, crypto and metals together",
        sankey: "Cashflow Sankey diagram",
        offline: "No cloud sync — prices need network",
      },
    },
    faq: {
      heading: "Common questions",
      items: {
        free: {
          q: "Is it really free?",
          a: "Yes. Every feature is free, with no signup, no ads and no premium tier. Use the web app or the native Linux and Android builds.",
        },
        storage: {
          q: "Where does my data live?",
          a: "On your device only, encrypted with AES-GCM. Nothing is uploaded or cloud-synced. Export a full backup anytime and restore it elsewhere.",
        },
        assets: {
          q: "What can I track?",
          a: "Stocks, ETFs, crypto, precious metals, cash accounts and custom holdings, with live prices from public feeds or your own Finnhub key.",
        },
        account: {
          q: "Do I need an account?",
          a: "Never. Open the app and start tracking. No login, no email, no telemetry. Walk away with your data in one export.",
        },
        platforms: {
          q: "Which platforms are available?",
          a: "The web app works in any modern browser. Native builds ship today for Linux (AppImage, .deb, .rpm) and Android. Windows, macOS and iOS are coming soon.",
        },
        license: {
          q: "Is the source code available?",
          a: "Yes. Aurelia is free software under AGPL-3.0-or-later. You can read, audit and contribute on GitHub.",
        },
      },
    },
    cta: {
      heading: "Take back your money in 60 seconds",
      subheading: "No signup. No card. Open Aurelia and see what you already own.",
      button: "Open Aurelia",
      footnote: "Works in any modern browser · Linux and Android apps available now · Windows, macOS and iOS coming soon",
    },
    downloads: {
      heading: "Take it with you",
      subheading:
        "Native packages from GitHub Releases. Same private app as the web — data stays on your device.",
      web: "Or open it in your browser",
      soon: "Coming soon",
      comingSoon: "Coming soon",
      action: "Download",
      recommended: "For this device",
      unsignedNotice:
        "Builds are unsigned and distributed directly from GitHub Releases. Your OS may warn on first launch. That is expected.",
      platforms: {
        windows: "Windows",
        mac: "macOS",
        linuxDeb: "Linux .deb",
        linuxRpm: "Linux .rpm",
        linuxAppImage: "Linux AppImage",
        android: "Android",
        ios: "iOS",
      },
      notes: {
        windows: "SmartScreen: More info > Run anyway",
        mac: "Right-click > Open on first launch",
        linuxDeb: "Debian, Ubuntu, Mint",
        linuxRpm: "Fedora, RHEL, openSUSE",
        linuxAppImage: "Portable, any distro",
        android: "Allow installs from unknown sources",
        ios: "Requires AltStore or Sideloadly",
      },
    },

    footer: {
      brand: "Aurelia",
      madeBy: "Made by",
      sourceCode: "Source code",
    },
    meta: {
      title: "Aurelia — Private portfolio & net worth tracker (no cloud)",
      description:
        "Track stocks, ETFs, crypto and cash with encrypted, on-device storage. Sankey cashflow, budgets and net worth. Free web app plus Linux and Android. No signup, no cloud sync.",
      keywords:
        "aurelia, private portfolio tracker, net worth tracker, local finance app, no cloud sync, stock tracker, crypto portfolio, cashflow sankey, budget planner, AGPL finance app",
    },
  },
  dashboard: {
    metaTitle: "Dashboard · Aurelia",
    metaDesc: "Your portfolio at a glance: allocation, net worth, and recent activity.",
    title: "Dashboard",
    description: "Welcome to your portfolio.",
    greeting: "Hi there",
    greetingNamed: "Hi {{name}}",
    greetingShort: "Hi there",
    headerNetWorth: "{{value}} net worth",
    netWorthSub: "Holdings plus liquidity, minus card debt",
    portfolioValue: "Portfolio value",
    netWorth: "Net worth",
    netWorthLiquidity: "{{sign}}{{value}} liquidity",
    liquidity: "liquidity",
    net30: "Last 30 days",
    net30Sub: "Net income minus expenses",
    net30Positive: "+{{value}} last 30 days",
    net30Negative: "-{{value}} last 30 days",
    topAsset: "Top asset",
    allocation: "Allocation",
    showAllLabels: "Show all labels",
    showAll: "Show all",
    hideAll: "Hide all",
    allAssetsHidden: "All assets are hidden.",
    showAllAssets: "Show all assets",
    addAsset: "Add asset",
    addEntry: "Log spending",
    holdingsCount: "{{count}} holdings",
    holdingsCountOne: "1 holding",
    topHolding: "Top: {{name}} ({{pct}})",
    emptyTitle: "Nothing here yet",
    emptyDescription:
      "Log a coffee or add your first ETF. Everything stays on this device, encrypted and yours.",
    emptyAction: "Add a holding",
    fabLog: "Log spending",
    localFirstHint: "On your device only",
  },
  holdings: {
    metaTitle: "Holdings · Aurelia",
    metaDesc: "Manage your stocks, crypto, ETFs and metals.",
    title: "Holdings",
    description: "Manage your stocks, crypto, ETFs and metals.",
    headerSubtitle: "{{count}} positions · {{value}}",
    fabAdd: "Add holding",
    positionsCount: "positions",
    addHolding: "Add holding",
    refresh: "Refresh prices",
    refreshed: "Prices refreshed",
    refreshFailed: "Failed to refresh some prices",
    symbol: "Symbol",
    quantity: "Quantity",
    shares: "Shares",
    amount: "Amount",
    manualPrice: "Manual price",
    price: "Price",
    value: "Value",
    invested: "Invested",
    positionsValue: "Positions value",
    transactions: "Transactions",
    addTransaction: "Add transaction",
    buy: "Buy",
    sell: "Sell",
    totalValue: "Total value",
    unitPrice: "Unit price",
    notes: "Notes",
    fees: "Fees",
    removed: "Removed {{symbol}}",
    dialog: {
      editTitle: "Edit holding",
      addTitle: "Add holding",
      description:
        "Search a market asset, or add a custom holding (e.g. Quanloop, private equity).",
      searchFailed: "Search failed — try a different proxy in Settings",
      qtyGtZero: "Quantity must be > 0",
      nameRequired: "Name is required",
      pickAsset: "Pick an asset first",
      customUpdated: "Custom holding updated",
      customAdded: "Custom holding added",
      holdingUpdated: "Holding updated",
      holdingAdded: "Holding added",
      priceFetchFailed: "Couldn't fetch price — you can refresh later",
      loadedPricePoints: "Loaded {{count}} price points",
      saveChanges: "Save changes",
      tabStock: "Stocks / ETF",
      tabCrypto: "Crypto",
      tabCustom: "Custom",
      selected: "Selected:",
      priceCurrency: "Price currency",
      autoDetectedHint: "auto-detected on save; override if needed",
      convertedHint: "will be converted to {{currency}}",
      horizon: "Horizon",
      horizonLong: "Long-term",
      horizonShort: "Short-term / cash-like",
      horizonHint:
        "Short-term holdings (e.g. lending platforms, broker cash) can be used as transfer accounts in Cashflow.",
      importCsv: "Import CSV",
      historyHint: "One row per snapshot: date,price. Latest entry becomes current price.",
      historyParsed: "Parsed {{count}} points · latest {{price}}",
      currentPrice: "Current price",
      manualPriceOptional: "Manual price (optional)",
      latestUnitPrice: "Latest known unit price",
      overrideLivePrice: "Override live price",
      symbolTicker: "Symbol / Ticker",
      notesOptional: "Notes (optional)",
      priceHistoryOptional: "Price history (optional)",
    },
    txDialog: {
      editTitle: "Edit transaction",
      addTitle: "Add transaction",
      description:
        "Record a buy or sell. Quantity on the holding is recalculated from your transactions.",
      pickHolding: "Pick a holding",
      pickHoldingError: "Pick a holding",
      qtyGtZero: "Quantity must be > 0",
      priceGteZero: "Price must be ≥ 0",
      updated: "Transaction updated",
      buyAdded: "Buy added",
      sellAdded: "Sell added",
      holding: "Holding",
      date: "Date",
      pricePerUnit: "Price / unit",
      totalValueAuto: "auto from qty × price",
      feesOptional: "Fees (optional)",
      notesOptional: "Notes (optional)",
      add: "Add",
    },
  },
  performance: {
    metaTitle: "Performance · Aurelia",
    metaDesc: "Track returns over time across your assets.",
    title: "Performance",
    description: "Historical portfolio value, period over period.",
    emptyState: "Add holdings first to see your historical performance.",
  },
  cashflow: {
    metaTitle: "Cashflow · Aurelia",
    metaDesc: "Track income, expenses, savings and investments.",
    title: "Cashflow",
    description: "Track every euro in and out, and see where it goes.",
    addData: "Add data",
    addDataHint: "Log income, spending, or a transfer",
    fabAdd: "Add data",
    income: "Income",
    expense: "Expense",
    expenses: "Expenses",
    net: "Net",
    source: "Source",
    addEntry: "Add entry",
    editEntry: "Edit entry",
    flow: "Flow",
    entries: "Entries",
    recurrence: "Recurrence",
    none: "One-time",
    weekly: "Weekly",
    monthly: "Monthly",
    yearly: "Yearly",
    until: "Until",
    fixed: "Fixed",
    percent: "Percent",
    percentOf: "Percentage of",
    allIncome: "All income",
    allExpenses: "All expenses",
    filters: "Filters",
    period: "Period",
    week: "Week",
    month: "Month",
    year: "Year",
    custom: "Custom",
    exportPdf: "Export PDF",
    exportingPdf: "Exporting PDF…",
    pdfReportTitle: "Cashflow Report",
    pdfPeriod: "Period",
    pdfFilters: "Type: {{type}} · Category: {{category}} · Currency: {{currency}}",
    pdfIncome: "Income",
    pdfExpenses: "Expenses",
    pdfNet: "Net",
    pdfDate: "Date",
    pdfType: "Type",
    pdfSourceCategory: "Source / Category",
    pdfAmount: "Amount",
    pdfInCurrency: "In {{currency}}",
    pdfBalancePositive: "Balance ≥ 0",
    pdfBalanceNegative: "Balance < 0",
    allTime: "All time",
    labels: { always: "Always", hover: "On hover", off: "Off" },
    labelMode: "Labels",
    customColors: "Custom colors",
    categories: "Categories",
    manageCategories: "Manage categories",
    newCategory: "New category",
    transferTab: "Transfer",
    descriptionLabel: "Description (optional)",
    descriptionPlaceholder: "e.g. groceries at Migros",
    transferDescPlaceholder: "e.g. Quanloop withdrawal to pay Amex",
    paidWith: "Paid with",
    liquidityCash: "Liquidity (cash)",
    holdingPrefix: "Holding",
    cardPrefix: "Credit card",
    splitInstallments: "Split into installments (financing)",
    payments: "# Payments",
    every: "Every",
    firstDue: "First due",
    weekOpt: "Week",
    monthOpt: "Month",
    repeats: "Repeats automatically",
    frequency: "Frequency",
    untilOptional: "Until (optional)",
    percentToggle: "Use a percentage of another entry (e.g. taxes)",
    transferIntro:
      "Move money between accounts without affecting income or expense totals. E.g. selling part of an investment to pay a credit card, or paying down card debt from liquidity.",
    from: "From",
    to: "To",
    addIncomeBtn: "Add income",
    addExpenseBtn: "Add expense",
    addTransferBtn: "Add transfer",
    addRecurringIncome: "Add recurring income",
    addRecurringExpense: "Add recurring expense",
    addFinancedExpense: "Add financed expense",
    emptyFlow: "Add some income and expenses to see the flow.",
    pickCategory: "Pick a category",
    selectCategory: "Select a category",
    newSourceOption: "New source…",
    newCategoryOption: "New category…",
    categoryNameIncomePlaceholder: "e.g. Bonuses",
    categoryNameExpensePlaceholder: "e.g. Subscriptions",
    amountGtZero: "Amount must be > 0",
    percentTooHigh: "Percentage looks too high",
    pickBothAccounts: "Pick both accounts",
    accountsMustDiffer: "Accounts must differ",
    incomeAdded: "Income added",
    expenseAdded: "Expense added",
    transferAdded: "Transfer added",
    percentTargetIncomeAll: "All income (total)",
    percentTargetExpenseAll: "All expenses (total)",
    incomeEntriesGroup: "Income entries",
    expenseEntriesGroup: "Expense entries",
    deletedEntry: "(deleted entry)",
    unnamed: "(unnamed)",
    percentOfShort: "Percent of",
    other: "Other",
    cashPool: "Cash Pool",
    resetLastMonth: "Reset last month",
    resetLastMonthTitle: "Delete last month's entries?",
    resetLastMonthDesc:
      "This permanently removes every cashflow entry dated in {{range}}. Recurring rules are kept.",
    resetLastMonthDone: "Removed {{count}} entries",
    breakdown: {
      title: "Breakdown by category",
      incomes: "Incomes",
      expenses: "Expenses",
      investments: "Investments & Savings",
    },
    upcoming: {
      title: "Upcoming",
      description: "Scheduled income and payments over the next few weeks.",
      tabOverview: "Overview",
      tabUpcoming: "Upcoming",
      period7: "7 days",
      period14: "2 weeks",
      period30: "1 month",
      periodShort7: "7d",
      periodShort14: "2w",
      periodShort30: "1mo",
      periodShort90: "90d",
      viewAll: "View all upcoming",
      today: "Today",
      tomorrow: "Tomorrow",
      expectedIn: "Expected in the next {{days}} days",
      empty: "Nothing scheduled ahead",
      emptyHint: "Add a recurring entry or schedule a future-dated payment to see it here.",
      recurring: "Recurring",
      oneTime: "One-time",
      whenLabel: "When",
      startDate: "Start date",
      previewRecurring: "Every {{frequency}} on day {{day}}",
    },
    sankey: {
      totalIncome: "Total Income",
      totalExpenses: "Total Expenses",
      totalSavings: "Total Savings",
      totalInvestments: "Total Investments",
      saved: "Saved",
      deficit: "Deficit",
      layoutStaged: "Grouped",
      layoutClassic: "Accounts",
      stageCategories: "Categories",
      stageDescriptions: "Descriptions",
      general: "General",
    },
  },
  cards: {
    title: "Credit cards",
    add: "Add card",
    empty:
      "No cards yet. Add a card to track debt from credit purchases and pay it down with transfers.",
    balanceOwed: "Balance owed",
    limit: "Limit",
    available: "Available",
    statementDayAbbr: "stmt day",
    dueDayAbbr: "due day",
    pay: "Pay",
    payAria: "Pay card from liquidity",
    editAria: "Edit card",
    deleteAria: "Delete card",
    removeConfirm: 'Remove "{{name}}"?',
    used: "{{pct}}% used",
    overLimit: "over limit",
    newCard: "New credit card",
    editCard: "Edit credit card",
    dialogDescription:
      "Track balance and available credit. Charges and payments come from your cashflow entries.",
    name: "Name",
    namePlaceholder: "Amex Gold",
    currency: "Currency",
    creditLimit: "Credit limit",
    statementDayLabel: "Statement day",
    dueDayLabel: "Due day",
    color: "Color",
    cancel: "Cancel",
    save: "Save",
    nameRequired: "Name required",
    cardAdded: "Card added",
    cardUpdated: "Card updated",
    cardRemoved: "Card removed",
    payTitle: "Pay {{name}}",
    payDesc: "Records a transfer that reduces this card's balance owed. Current debt:",
    payFrom: "Pay from",
    amount: "Amount",
    recordPayment: "Record payment",
    paymentCategory: "Card payment",
    paymentTo: "Payment to {{name}}",
    paymentRecorded: "Recorded payment to {{name}}",
  },
  admin: {
    title: "Admin - Sponsors",
    addSponsor: "Add sponsor",
    sponsorsSaved: "Sponsors saved to GitHub",
    wrongPassword: "Wrong password",
    enter: "Enter",
    logoUrl: "Logo URL",
    tagline: "Tagline",
    weight: "Weight (1–10)",
    schedule: "Schedule (optional)",
    active: "Active",
    paused: "Paused",
    needName: "Each sponsor needs a name",
  },
  settings: {
    metaTitle: "Settings · Aurelia",
    metaDesc: "Profile, currency, backup, and on-device AI.",
    title: "Settings",
    description: "Your profile, currency, and data vault.",
    sections: {
      profile: "Profile",
      appearance: "Look",
      categories: "Categories",
      currency: "Currency",
      data: "Data",
      ai: "AI",
    },
    appearance: {
      title: "Look & feel",
      description: "Light or dark, a color palette, and the guided tour.",
    },
    categories: {
      title: "Cashflow categories",
      description: "Income sources and expense groups used when you add entries.",
      manage: "Manage categories",
    },
    api: {
      title: "API & reliability",
      description: "CoinGecko and Yahoo Finance work without keys. Finnhub is optional fallback.",
      displayCurrency: "Display currency",
      displayCurrencyHelp:
        "All values across the app are converted to this currency using live FX rates (cached for 6 hours).",
      privacyMode: "Privacy mode",
      privacyModeHelp:
        "Hide quantities and dollar values across the app (the eye icon in the header does the same thing).",
      useCorsProxy: "Use CORS proxy",
      useCorsProxyHelp: "Enable if Yahoo Finance requests are blocked in your browser.",
      corsProxyUrl: "CORS proxy URL",
      finnhubKey: "Finnhub API key (optional)",
      finnhubPlaceholder: "paste key from finnhub.io",
      finnhubFootnote: "Free at finnhub.io. Used as fallback for stocks when Yahoo fails.",
      saved: "Saved",
      clearPriceCache: "Refresh price history",
      clearPriceCacheHelp:
        "Drop cached price data so the next chart load re-fetches from the providers.",
      priceCacheCleared: "Price cache cleared",
    },
    ai: {
      title: "AI Assistant",
      description:
        "Fully offline. Configure local models for the LLM and voice, or use the built-in assistant with no download.",
      enabled: "Show AI assistant",
      enabledHelp: "Hide the assistant from the navigation menu to free up space.",
      voiceOutput: "Speak replies aloud",
      voiceOutputHelp: "Read the assistant's answers using on-device text-to-speech.",
      llmModel: "Language model (GGUF)",
      llmModelHelp:
        "e.g. Qwen2.5-1.5B-Instruct Q4_K_M. Without one, the built-in assistant is used.",
      sttModel: "Speech-to-text model folder",
      sttModelHelp: "A Sherpa-ONNX STT model folder. Falls back to the browser recognizer.",
      ttsModel: "Text-to-speech model folder",
      ttsModelHelp: "A Sherpa-ONNX VITS/Piper model folder. Falls back to the browser voice.",
      webNote:
        "You're on the web build. The assistant works offline with the built-in engine and your browser's voice. Install the desktop/mobile app to run local LLM and Sherpa-ONNX speech models.",
      clearHistory: "Clear chat history",
      clearHistoryHelp: "Delete the saved AI conversation from this device.",
      historyCleared: "Chat history cleared",
      browse: "Browse",
      clearPath: "Clear",
      pickError: "Couldn't open the file picker.",
      download: "Download",
      downloadStarting: "Starting download…",
      downloadProgress: "{{pct}}% downloaded",
      downloadProgressUnknown: "{{mb}} MB downloaded",
      downloadExtracting: "Extracting…",
      downloadReady: "Download complete",
      downloadFailed: "Model download failed",
      downloadRetry: "Try again",
      setupTitle: "Download local AI models?",
      setupDescription:
        "Optional offline upgrades (~{{size}} total). The built-in assistant works without them. Downloads stay on this device.",
      setupDownloadAll: "Download all",
      setupDecline: "Not now",
      setupDone: "Models installed. Paths were set automatically.",
    },
    language: {
      title: "Language",
      description: "Choose the language for the entire app.",
      label: "Interface language",
      auto: "Auto (browser default)",
    },
    data: {
      title: "Data",
      description: "All state lives in your browser localStorage. Back up or move it anytime.",
      exportJson: "Export full state (JSON)",
      exportCsv: "Export holdings (CSV)",
      importJson: "Import JSON",
      reset: "Reset all data",
      resetTitle: "Delete everything?",
      resetDesc:
        "This removes all holdings, cashflow entries, and settings from this browser. Export a backup first if you want to keep it.",
      resetConfirm: "Yes, delete",
      imported: "Data imported",
      importFailed: "Couldn't import",
      cleared: "All data cleared",
      exported: "Export completed",
      exportFailed: "Export failed",
      exportedFile: "Saved {{filename}}",
      exportedShare: "Shared {{filename}}",
      exportedClipboard: "Copied to clipboard (download unavailable on this device)",
      exportedNative: "Saved {{filename}}",
      exportCancelled: "Export cancelled",
      copied: "JSON copied to clipboard",
      copyFailed: "Copy failed",
      copyJson: "Copy JSON to clipboard",
      pasteJson: "Paste JSON to import",
      pasteJsonDesc: "Paste a previously exported JSON below. This will replace your current data.",
      pasteFromClipboard: "Paste from clipboard",
      clipboardReadFailed: "Could not read clipboard",
    },
    about: {
      title: "About",
      body: "Aurelia is fully client-side. Prices come from CoinGecko (crypto) and Yahoo Finance (stocks, ETFs, metals). No account, no backend.",
      metalsHint: "For metals use Yahoo symbols like GC=F (gold) or SI=F (silver).",
    },
  },
  chart: {
    screenshotSaved: "Screenshot saved",
    screenshotFailed: "Couldn't capture screenshot",
  },
  more: {
    entriesFiltersType: "Type",
    entriesFiltersCategory: "Category",
    entriesFiltersPeriod: "Period",
    entriesFiltersFrom: "From",
    entriesFiltersTo: "To",
    entriesAllCategories: "All categories",
    entriesThisWeek: "This week",
    entriesThisMonth: "This month",
    entriesThisYear: "This year",
    entriesAllTime: "All time",
    entriesCustomRange: "Custom range",
    entriesAll: "All",
    entriesIncome: "Income",
    entriesExpense: "Expense",
    entriesDate: "Date",
    entriesType: "Type",
    entriesSourceCategory: "Source / Category",
    entriesAmount: "Amount",
    entriesEmpty: "No entries match the filters.",
    entriesNoChart: "No data for the selected filters.",
    entriesNoneInRange: "No entries in selected range",
    entriesEditAria: "Edit entry",
    entriesDeleteAria: "Delete entry",
    entriesDeleteRecurringConfirm: "Delete the entire recurring entry and all its occurrences?",
    entriesUpdated: "Entry updated",
    entriesPdfExported: "PDF exported",
    entriesPdfTitle: "Cashflow Report",
    entriesHeaderIn: "In {{currency}}",
    entriesCount: "{{count}} entries",
    entriesIncomeLabel: "Income",
    entriesExpensesLabel: "Expenses",
    entriesNetLabel: "Net",
    entriesBalance: "Balance",
    entriesNoActivity: "No activity",
    entriesOf: "of",
    mcTrigger: "Categories",
    mcTitle: "Manage categories",
    mcDesc:
      "Organize income sources and expense categories. Savings and Investments are expense-side categories with their own coloring.",
    mcAddNew: "Add new",
    mcNamePlaceholder: "Category name",
    mcAdd: "Add",
    mcIncome: "Income",
    mcExpense: "Expense",
    mcSavings: "Savings",
    mcInvestment: "Investment",
    mcIncomeHeader: "Income",
    mcExpensesHeader: "Expenses",
    mcNameRequired: "Name required",
    skLabelsAlways: "Labels: always",
    skLabelsHover: "Labels: hover",
    skLabelsOff: "Labels: off",
    skCustomizeColors: "Customize colors",
    skNodeColors: "Node colors",
    skReset: "Reset",
    pcTitle: "Portfolio over time",
    pcValuePerAsset: "Value per asset",
    pcInvestedVsValue: "Invested vs Value",
    pcAssets: "Assets",
    pcShowAll: "Show all",
    pcHideAll: "Hide all",
    pcInvested: "Invested",
    pcValue: "Value",
    pcGain: "Gain",
    tpTitle: "Transactions",
    tpType: "Type",
    tpHolding: "Holding",
    tpPeriod: "Period",
    tpAll: "All",
    tpBuy: "Buy",
    tpSell: "Sell",
    tpAllHoldings: "All holdings",
    tpAllTime: "All time",
    tpThisWeek: "This week",
    tpThisMonth: "This month",
    tpThisYear: "This year",
    tpCustomRange: "Custom range",
    tpFrom: "From",
    tpTo: "To",
    tpDate: "Date",
    tpQuantity: "Quantity",
    tpPrice: "Price",
    tpFees: "Fees",
    tpTotal: "Total",
    tpRemoved: "Transaction removed",
    tpAddTransaction: "Add transaction",
    tpInvested: "Invested",
    tpProceeds: "Proceeds",
    tpFeesLabel: "Fees",
    tpNetInvested: "Net invested",
    tpCountSuffix: "transactions",
    tpEmpty: "No transactions yet. Add a buy or sell to start tracking.",
    tpEmptyFilters: "No transactions match the filters.",
    tpEdit: "Edit",
    tpDelete: "Delete",
    perfPortfolioValue: "Portfolio value",
    perfAsset: "Asset",
    perfStart: "Start",
    perfEnd: "End",
    perfChange: "Change",
    perfTotal: "Total",
    dashBreakdown: "Breakdown",
    dashNoHoldings: "No holdings yet",
    dashHoldings: "Holdings",
    hSearch: "Search symbol or name…",
    hAllTypes: "All types",
    hTypeStock: "Stocks",
    hTypeEtf: "ETFs",
    hTypeCrypto: "Crypto",
    hTypeMetal: "Metals",
    hTypeOther: "Other",
    hType: "Type",
    hName: "Name",
    hSymbol: "Symbol",
    hQuantity: "Quantity",
    hPrice: "Price",
    hValue: "Value",
    hManual: "man",
    hNoMatch: "No holdings match your filters.",
    perfReturnsByAsset: "Returns by asset",
    perfCouldntLoad: "Couldn't load price history. Try enabling the CORS proxy in Settings.",
    perfNoData: "No data",
    perfAssets: "Assets",
    perfTotalLabel: "Total",
  },

  tour: {
    start: "Take the tour",
    next: "Next",
    prev: "Back",
    done: "Finish",
    skip: "Skip",
    progress: "{{current}} of {{total}}",
    restartTitle: "Onboarding tour",
    restartDesc: "Replay the guided tour at any time to refresh your memory.",
    restartButton: "Start tour",
    steps: {
      welcome: {
        title: "Welcome to Aurelia",
        body: "This quick tour walks through the app: home, cashflow, holdings, planning, and more. Skip anytime, or replay from Settings.",
      },
      sidebar: {
        title: "Main navigation",
        body: "Jump between Dashboard, Holdings, Performance, Cashflow, Planning, Assistant, and Settings from this sidebar.",
      },
      mobileNav: {
        title: "Quick navigation",
        body: "Use the bottom bar: Home, Flow, Assets. Tap More for Performance, Planning, Assistant, and Settings.",
      },
      tourLauncher: {
        title: "Replay this tour",
        body: "Tap the help icon anytime to restart the guided tour. You can also start it from Settings.",
      },
      theme: {
        title: "Light & dark mode",
        body: "Switch between light, dark, or system theme. Your choice is remembered on this device.",
      },
      privacy: {
        title: "Privacy mode",
        body: "Hide all monetary values with one tap — ideal for screen sharing or public charts.",
      },
      dashStats: {
        title: "Your top-line numbers",
        body: "Portfolio value, net worth (holdings + liquidity − card debt), and the last 30 days of cashflow.",
      },
      dashQuickActions: {
        title: "Quick actions",
        body: "Shortcuts to add an asset, log a cashflow entry, open Holdings, or jump to Performance.",
      },
      dashAllocation: {
        title: "Asset allocation",
        body: "See how your portfolio is distributed. Toggle assets in the legend or show all labels at once.",
      },
      dashBreakdown: {
        title: "Breakdown by asset",
        body: "A detailed list of every holding with its current weight in the portfolio.",
      },
      holdingsAdd: {
        title: "Add a holding",
        body: "Track stocks, ETFs, crypto, cash, metals, and custom assets. Live prices are fetched when possible.",
      },
      holdingsFilters: {
        title: "Search & filter",
        body: "Find any holding by name, symbol, or type, and sort the table to match how you work.",
      },
      holdingsTable: {
        title: "Your holdings",
        body: "See quantity, price, and value for each position. Open a row to edit it or log a buy/sell.",
      },
      holdingsCharts: {
        title: "Holdings charts",
        body: "Visualise value over time and invested cost basis so you can see how positions grow.",
      },
      holdingsTx: {
        title: "Buys & sells",
        body: "Log every transaction with fees. Quantities and cost basis stay in sync automatically.",
      },
      perfPeriod: {
        title: "Pick a period & scale",
        body: "Compare over 1D to MAX. Switch between absolute value and indexed % to compare returns fairly.",
      },
      perfAssets: {
        title: "Toggle assets",
        body: "Show or hide the total line and individual assets to focus on what matters.",
      },
      perfChart: {
        title: "Portfolio over time",
        body: "Hover or tap the chart to inspect exact values across the selected period.",
      },
      perfReturns: {
        title: "Returns by asset",
        body: "See start value, end value, and absolute and percentage change for each holding.",
      },
      cfSummary: {
        title: "Cashflow snapshot",
        body: "Income, expenses, and net for the same period as the flow diagram below.",
      },
      cfAdd: {
        title: "Add income, expenses & transfers",
        body: "Log one-time or recurring entries, use percent-of rules, installments, and pay from cash or a card.",
      },
      cfCards: {
        title: "Credit cards & debt",
        body: "Track limits and balances, and pay down cards. Card-paid expenses appear in the flow diagram too.",
      },
      cfSankey: {
        title: "Money flow diagram",
        body: "Watch money move from income to expenses, savings, and investments. Drag nodes to reorder.",
      },
      cfBreakdown: {
        title: "Category breakdown",
        body: "Pie charts for income, expenses, and investments/savings so you see where money goes.",
      },
      cfEntries: {
        title: "All your entries",
        body: "Filter by type, category, and period, edit entries, or export a PDF with a balance chart.",
      },
      planTabs: {
        title: "Planning hub",
        body: "Four tools in one place: Forecast, Budgets, Goals, and Loans — all tied to your cashflow data.",
      },
      planForecast: {
        title: "Cashflow forecast",
        body: "Project the next 24 months from recurring entries. Create scenarios, adjust income/expenses, and set a main plan.",
      },
      planBudgets: {
        title: "Budgets",
        body: "Build plans (monthly, vacation, project…) with category line items and track progress against real spending.",
      },
      planGoals: {
        title: "Savings goals",
        body: "Set a target amount and date. Progress bars and suggested monthly savings keep you on track.",
      },
      planLoans: {
        title: "Loans",
        body: "Add loans and review amortization schedules so you know interest and payoff timelines.",
      },
      assistantIntro: {
        title: "On-device AI assistant",
        body: "Chat locally about spending and budgets. Expense proposals need your confirmation before anything is saved.",
      },
      assistantChat: {
        title: "Conversation",
        body: "Ask for spending summaries, budget status, or recent transactions. Confirm any expense the assistant proposes.",
      },
      assistantInput: {
        title: "Text & voice",
        body: "Type a message or use the mic. Voice stays on-device when models are available on your platform.",
      },
      setApi: {
        title: "API & display options",
        body: "Choose display currency, privacy defaults, CORS proxy if needed, and optional Finnhub key for richer prices.",
      },
      settingsAi: {
        title: "AI settings",
        body: "Enable or hide the Assistant in navigation, toggle voice output, and manage on-device models — nothing leaves this device.",
      },
      setLanguage: {
        title: "Language",
        body: "Switch between English, Spanish, Catalan/Valencià, Portuguese, Dutch, and German. Your choice is saved with backups.",
      },
      setData: {
        title: "Backup & restore",
        body: "Your data stays local. Export JSON or CSV, import a backup, or reset everything from scratch.",
      },
      finish: {
        title: "You're all set!",
        body: "Explore Dashboard through Settings at your own pace. Replay this tour anytime from the help icon or Settings. Happy tracking!",
      },
    },
  },
  planning: {
    title: "Planning",
    description: "Budgets, goals, forecasts and loans - all derived from your existing cashflow.",
    head: {
      title: "Planning · Budgets, Goals, Forecast, Loans",
      description:
        "Plan your finances: monthly budgets, savings goals, cashflow forecasts and loan amortization.",
    },
    tabs: { budgets: "Budgets", goals: "Savings goals", forecast: "Forecast", loans: "Loans" },
    budgets: {
      addTitle: "Add monthly budget",
      category: "Category",
      pickCategory: "Pick a category",
      monthlyLimit: "Monthly limit",
      limitPlaceholder: "e.g. 500",
      currency: "Currency",
      addBudget: "Add budget",
      addBudgetItem: "Add budget item",
      addItemHint: "Set a limit for a category or a custom line item.",
      itemAdded: "Budget item added",
      thisMonth: "This month",
      empty: "No budgets yet. Add a monthly limit to a category to start tracking.",
      unknown: "Unknown",
      overBy: "Over by {{amount}}",
      left: "{{amount}} left",
      newPlanName: "New plan",
      untitledItem: "Untitled",
      noPlansHint:
        "Create a budget for anything — your regular monthly plan, a vacation, a personal project, a moving month. Add as many as you want.",
      newPlan: "New plan",
      total: "Total",
      isMain: "Main",
      setMain: "Set as main",
      edit: "Edit",
      duplicate: "Duplicate",
      deletePlanConfirm: "Delete this plan?",
      deletePlan: "Delete",
      itemLabel: "Label",
      labelPlaceholder: "e.g. Vacation fund",
      linkCategory: "Link to category (optional)",
      noCategory: "None — track manually",
      color: "Color",
      colorHint: "Optional — falls back to category",
      manual: "manual",
      notTracked: "Not tracked from cashflow",
      pieTitle: "Budget breakdown",
      pieEmpty: "Add items to see the breakdown",
      editPlan: "Edit plan",
      planDialogDesc: "Give this budget a clear name so you can tell it apart from your others.",
      planName: "Name",
      planNamePlaceholder: "e.g. Japan trip 2026",
      planDescription: "Description (optional)",
      planDescriptionPlaceholder: "What is this budget for?",
      period: "Period",
      periodDays: "Days",
    },
    goals: {
      newTitle: "New savings goal",
      name: "Name",
      namePlaceholder: "Emergency fund",
      targetAmount: "Target amount",
      targetPlaceholder: "10000",
      currency: "Currency",
      alreadySaved: "Already saved",
      targetDate: "Target date (optional)",
      addGoal: "Add goal",
      addHint: "Set a target and track how much you have saved.",
      added: "Goal added",
      empty: "No goals yet.",
      by: "by {{date}}",
      saveHint: "Save ~{{amount}}/mo to hit your goal.",
      addContribution: "Add contribution ({{currency}})",
    },
    forecast: {
      incomeMo: "Monthly income (recurring)",
      expenseMo: "Monthly expenses (recurring)",
      savingsRate: "Savings rate",
      liquidityForecast: "Liquidity forecast",
      months: "Months",
      projectedBalance: "Projected balance",
      runway:
        "At your current recurring spend you have ~{{months}} months of runway from today's liquidity.",
      recurringTitle: "Recurring subscriptions & income",
      recurringEmpty:
        "No recurring entries detected. Mark income/expenses as recurring on the Cashflow page.",
      perMo: "{{amount}}/mo",
      newScenarioName: "New scenario",
      noScenariosHint:
        "Create as many forecast scenarios as you want — one for your personal life, one for a side project, one for your small business. Compare them side by side.",
      newScenario: "New scenario",
      adjustments: "Applying adjustments: {{i}}/mo income, {{e}}/mo expense",
      isMain: "Main",
      setMain: "Set as main",
      edit: "Edit",
      duplicate: "Duplicate",
      deleteConfirm: "Delete this scenario?",
      delete: "Delete",
      monthlySnapshot: "Monthly snapshot",
      recurringIncome: "Recurring income",
      subscriptions: "Subscriptions & recurring expenses",
      piesTitle: "Recurring income & expenses breakdown",
      scenarioAdjustment: "Scenario adjustment",
      recurringExpenses: "Recurring expenses",
      perMonthShort: "Per month",
      editScenario: "Edit scenario",
      scenarioDialogDesc: "Give this scenario a name so you can compare it against others.",
      scenarioName: "Name",
      scenarioNamePlaceholder: "e.g. Small business",
      notes: "Notes",
      incomeAdjustPerMonth: "± income /mo",
      expenseAdjustPerMonth: "± expense /mo",
    },
    loans: {
      newTitle: "New loan",
      name: "Name",
      namePlaceholder: "Mortgage",
      principal: "Principal",
      apr: "APR %",
      termMonths: "Term (months)",
      start: "Start",
      extraOpt: "Extra monthly (optional)",
      notes: "Notes",
      currency: "Currency",
      addLoan: "Add loan",
      addHint: "Track a loan and see the amortization schedule.",
      added: "Loan added",
      empty: "No loans tracked yet.",
      hide: "Hide",
      schedule: "Schedule",
      monthlyPayment: "Monthly payment",
      totalInterest: "Total interest",
      remaining: "Remaining",
      payoff: "Payoff",
      paidDown: "{{pct}}% paid down",
      extraShort: "Extra: {{amount}}/mo",
      extraLabel: "Extra/mo",
      meta: "{{principal}} @ {{apr}}% APR · {{months}} mo · starts {{date}}",
      tbl: {
        n: "#",
        date: "Date",
        payment: "Payment",
        interest: "Interest",
        principal: "Principal",
        balance: "Balance",
      },
    },
  },
};

type DeepString<T> = { [K in keyof T]: T[K] extends object ? DeepString<T[K]> : string };
export type Translation = DeepString<typeof en>;
export default en as Translation;
